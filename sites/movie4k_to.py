...
####
URL_MAIN = f'http://www.{DOMAIN}'
URL_MOVIES = f'{URL_MAIN}/index.php'
URL_MOVIES_ALL = f'{URL_MAIN}/movies-all'
URL_MOVIES_GENRE = f'{URL_MAIN}/genres-movies.html'

URL_SERIES = f'{URL_MAIN}/tvshows_featured.php'
URL_SERIES_ALL = f'{URL_MAIN}/tvshows-all'
URL_SERIES_TOP = f'{URL_MAIN}/tvshows-top.html'
URL_SERIES_GENRE = f'{URL_MAIN}/genres-tvshows.html'

URL_XXX = f'{URL_MAIN}/xxx-updates.html'
URL_XXX_ALL = f'{URL_MAIN}/xxx-all'
URL_XXX_GENRE = f'{URL_MAIN}/genres-xxx.html'

URL_SEARCH = f'{URL_MAIN}/movies.php?list=search'
...
def __getHtmlContent(sUrl = False, sSecurityValue = False):
    ...
    # adult Cookie
    adultCookie = 'xxx2=ok;' if showAdult() else ''
    # Make the request
...
def __getPreferredLanguage():
    ...
    if sLanguage == '0':
        return 'lang=de;onlylanguage=de;'
    elif sLanguage == '1':
        return 'lang=us;onlylanguage=en;'
    else:
        return ''
...
def __clearProtection():
    oRequestHandler = cRequestHandler(f'{URL_MAIN}/index.php', False)
    oRequestHandler.removeNewLines(False)
    oRequestHandler.removeBreakLines(False)
    sHtmlContent = oRequestHandler.request()
    if result := jsunprotect.jsunprotect(sHtmlContent):
        logger.info(result)
        oRequestHandler = cRequestHandler(f'{URL_MAIN}?{result}', False)
        oRequestHandler.addHeaderEntry('Referer', URL_MAIN)
        oRequestHandler.addHeaderEntry('Host', f'www.{DOMAIN}')
...
def __createCharacters(oGui, sCharacter, sBaseUrl):
    ...
    if (sCharacter == '#'):
        sUrl = f'{sBaseUrl}-1-1.html'
    else:
        sUrl = f'{sBaseUrl}-{str(sCharacter)}-1.html'
...
def parseMovieSimpleList():
    ...
    iPage = params.getValue('iPage') if (params.exist('iPage')) else 1
    if (params.exist('sUrl')):
        sUrl = params.getValue('sUrl')
        logger.info(sUrl)
    ...
    if (params.exist('sUrl') and params.exist('sMovieTitle')):
        ...
        if (aResult[0] == True):
            ...
            for aEntry in aResult[1]:
                ...
                hoster = {'name': sHoster, 'link': f'{URL_MAIN}/{aEntry[0]}'}
    ...
    if (params.exist('sUrl') and params.exist('sMovieTitle')):
        ...
        if (aResult[0] == True):
            ...
            for aEntry in aResult[1]:
                ...
                hoster = {
                    'name': sHoster,
                    'link': f'{URL_MAIN}/{aEntry[0]}',
                    'displayedName': f'{aEntry[1]} - {sHoster} - Quality: {aEntry[3]}',
                    'quality': aEntry[3],
                    'date': aEntry[1].strip(),
                }
...
def showHoster(sUrl=False):
    ...
    if aResult[0]: # multipart stream
        for aEntry in aResult[1]:
            ...
            result['title'] = f'{sMovieTitle} Part {aEntry[2]}'
...
def __getMovieTitle(sHtmlContent):
    ...
    aResult = oParser.parse(sHtmlContent, 'Watch (.*?) online.*?</title>')
...
def __getLanguage(sString):
    return 'de' if 'us_ger_small' in sString else 'en'
